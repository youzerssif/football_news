...
def actu(link):
    ...
    if response.status_code == 200:
        ...
        autor_and_date = html_soup.find("div", attrs={"class": "post-info"}).text.strip().replace('\xa0',' ').replace('\n','')
        title = html_soup.find("h1", attrs={"class": "post-title"}).text
        image = html_soup.find("img")['src']
        short_desc = html_soup.find("div", attrs={"class": "post-excerpt"}).text.strip().replace('\xa0',' ').replace('\n','')
        description = html_soup.find("div", attrs={"class": "post-content"}).text.strip().replace('\xa0',' ').replace('\n','')
        article = {
